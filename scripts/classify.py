...
PIPELINE_VERSION = "2026-07-21-v5"
...
3. Определи строгость требования по опыту (strict: true/false) —
   ...
   тегом вида "Грейд: #junior" / "Грейд: #middle" / "Грейд: #senior"
   (или "Level: Senior" и т.п.). Это ПРЯМОЙ источник для
   experience.value — используй его, даже если по ссылке не удалось
   получить дополнительных деталей. Не пиши null в experience.value,
   если грейд явно указан таким тегом в тексте поста — это такая же
   валидная информация, как обычная фраза "требуется опыт от 3 лет".
